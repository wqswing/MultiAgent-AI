...
use multi_agent_governance::{
    AuditEntry, AuditFilter, AuditOutcome, AuditStore, InMemoryAuditStore,
    NoOpRbacConnector, RbacConnector, UserRoles,
};
...
async fn test_rbac_noop_connector_allows_all() {
    ...
    let allowed = connector.check_permission("token", "any_resource", "any_action").await.unwrap();
    assert!(allowed);
}
...
async fn test_audit_store_log_and_query() {
    ...
    // Query by user
    let admin_entries = store.query(AuditFilter {
    ...
    // Query by action
    let tool_entries = store.query(AuditFilter {
