...
impl PricingRegistry {
    ...
    pub fn with_defaults() -> Self {
        ...
        // Anthropic models
        registry.register(ModelPricing::new("anthropic:claude-3-haiku-20240307", 0.25, 1.25).with_quality(7));
        registry.register(ModelPricing::new("anthropic:claude-3-5-sonnet-20241022", 3.00, 15.00).with_quality(10));
...
impl SessionCostTracker {
    ...
    pub fn is_over_budget(&self) -> bool {
        self.budget_limit.map_or(false, |limit| self.accumulated_cost >= limit)
    }
    
    /// Get remaining budget.
    pub fn remaining_budget(&self) -> Option<f64> {
        self.budget_limit.map(|limit| (limit - self.accumulated_cost).max(0.0))
    }
    
    /// Get usage percentage.
    pub fn usage_percentage(&self) -> Option<f64> {
        self.budget_limit.map(|limit| (self.accumulated_cost / limit) * 100.0)
    }
}
