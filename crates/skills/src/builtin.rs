...
impl Tool for ReadArtifactTool {
    ...
    async fn execute(&self, args: Value) -> Result<ToolOutput> {
        let ref_id_str = args
            .get("ref_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| mutilAgent_core::Error::invalid_request("ref_id is required"))?;
...
impl Tool for CalculatorTool {
    ...
    async fn execute(&self, args: Value) -> Result<ToolOutput> {
        let operation = args
            .get("operation")
            .and_then(|v| v.as_str())
            .ok_or_else(|| mutilAgent_core::Error::invalid_request("operation is required"))?;

        let a = args
            .get("a")
            .and_then(|v| v.as_f64())
            .ok_or_else(|| mutilAgent_core::Error::invalid_request("a must be a number"))?;

        let b = args
            .get("b")
            .and_then(|v| v.as_f64())
            .ok_or_else(|| mutilAgent_core::Error::invalid_request("b must be a number"))?;
...
pub struct ListToolsTool {
    registry: Arc<dyn mutilAgent_core::traits::ToolRegistry>,
}

impl ListToolsTool {
    /// Create a new list tools tool.
    pub fn new(registry: Arc<dyn mutilAgent_core::traits::ToolRegistry>) -> Self {
        Self { registry }
    }
}
